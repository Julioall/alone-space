...
def draw_bars(screen, energy, heart, shield):
    ...
    screen.draw.filled_rect(Rect(10, 10, heart * (energy_bar_width / MAX_HEALTH), energy_bar_height), RED)  # Vermelho para vida
    screen.draw.filled_rect(Rect(10, 20, shield * (energy_bar_width / MAX_SHIELD), energy_bar_height), BLUE)  # Azul para escudo
    screen.draw.filled_rect(Rect(10, 30, energy * (energy_bar_width / MAX_ENERGY), energy_bar_height), GREEN)  # Verde para energia
...
def handle_shooting(shooting, energy, shoot_timer, bullets, player, bullet_animations, sounds, audio_on):
    if shooting and energy >= ENERGY_COST_BULLET:
        ...
        if shoot_timer >= SHOOT_INTERVAL:
            new_bullet = Actor("bullet-1", (player.x, player.y - (TILE_SIZE / 4)))
